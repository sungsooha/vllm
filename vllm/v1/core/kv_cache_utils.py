...
def resolve_kv_cache_block_sizes(
    ...
    - ``scheduler_block_size`` is the token-alignment invariant used by the
      ...
      that group's DCP-effective block size. Multiple groups: LCM of every
      group's DCP-effective block size.
    - ``hash_block_size`` is the granularity at which ``Request.block_hashes``
      ...
      is inactive or a non-DCP mamba group's block size diverges from the cache
