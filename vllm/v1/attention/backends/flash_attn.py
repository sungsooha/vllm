...
class FlashAttentionMetadataBuilder(AttentionMetadataBuilder[FlashAttentionMetadata]):
    ...
    def build(
        ...
        def schedule(
            ...
            if aot_schedule:
                # Under TPA-GQA the pre-attention Q all-gather across DCP is
                # skipped (Q already has H/attn_tp = H*DCP/TP heads per rank),
                # so the AOT scheduler must see H/attn_tp, not H/attn_tp * DCP.
                # Without TPA, Q is expanded by the all-gather and the
                # scheduler needs the expanded count.
...
class FlashAttentionImpl(AttentionImpl):
    ...
    def _forward_with_dcp(
        ...
        # Under TPA-GQA, GPUs in the same DCP group already have the local
        # H/attn_tp Q heads for the full context — no cross-DCP all-gather
        # needed. dcp_combine below reduce-scatters H/attn_tp → H/full_tp.
        from vllm.distributed.parallel_state import is_tpa_gqa_mode
        ...
        # FA3 with out= may return the underlying base buffer rather than the
        # sliced view. If the workspace pool sized for the larger non-TPA path
        # bleeds through, slice back to the actual head count used here.
        ...
        # Under TPA-GQA the query branch has H/attn_tp heads but the context
        # branch (after dcp_combine reduce-scatter) has H/full_tp heads. Slice
        # the query branch to this rank's DCP-rank head window so merge_attn_states
        # sees matching shapes.
        if tpa_gqa_mode:
