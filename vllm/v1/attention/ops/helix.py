...
# ============================================================================
# Static Buffers for Helix A2A
# ============================================================================
# Pre-allocate recv and output buffers to avoid per-layer torch.empty() calls.
# All 30 layers share the same buffers (they execute sequentially).
# CUDA graph warmup uses padded batch sizes, so max B is seen early.
...
def _ensure_a2a_buffers(
    ...
    _helix_a2a_buffers = {
        "recv_output": torch.empty(
            (N, B, H_per_rank, D), dtype=out_dtype, device=device
        ),
        "recv_lse": torch.empty((N, B, H_per_rank), dtype=lse_dtype, device=device),
        "out": torch.empty((B, H_per_rank, D), dtype=out_dtype, device=device),
        "out_lse": torch.empty((B, H_per_rank), dtype=lse_dtype, device=device),
    }
    logger.debug(
        "Helix A2A: allocated static buffers for B_max=%d, N=%d, H=%d, D=%d",
        B,
        N,
        H_per_rank,
        D,
...
def helix_alltoall_lse_reduce(
    ...
    Static buffers are pre-allocated for recv and output tensors to avoid
    per-layer torch.empty() overhead (120 allocations/step -> 0).
    ...
    # Ensure static buffers are allocated for the largest B seen.
    # CUDA graph warmup runs padded batch sizes in ascending order,
    # so buffers grow to max during warmup and stay fixed after.
    ...
    # Use static recv buffers when B matches buffer size (CUDA graph path).
    # During warmup with smaller B, fall back to fresh allocation since
    # all_to_all_single requires equal-sized input and output tensors.
    use_static = _helix_a2a_max_B == B
    if use_static:
        recv_output = bufs["recv_output"]
        recv_lse = bufs["recv_lse"]
    else:
    ...
    # This ensures recv buffers are fully populated before
    # the Triton kernel reads from them.
    work_output.wait()
    work_lse.wait()

    # Step 3: LSE-weighted combination via Triton kernel (LOCAL, no communication)
    # Use static output buffers when available to avoid per-call allocation.
    if use_static:
        out = bufs["out"]
        out_lse = bufs["out_lse"] if return_lse else None
    else:
        out = None
        out_lse = None
